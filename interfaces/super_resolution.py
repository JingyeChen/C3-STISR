...
ABLATION_SET = ["tsrn_tl_cascade", "srcnn_tl", "srresnet_tl", "rdn_tl", "vdsr_tl","c3stisr"]
            ...
            for j, data in (enumerate(train_loader)):
                ...
                if not self.args.go_test:
                    ...
                    if self.args.syn:
                        ...
                        if self.args.arch in ["tsrn","transstisr","tbsrn"]:
                    ...
                    if self.args.arch in ["tsrn","transstisr","tbsrn"]:
                        ...
                        for i in range(self.args.stu_iter):
                            ...
                            if self.args.arch in ['c3stisr']:
                                ...
                                lm_out = model_list[pick].module.lm(lm_input, lengths_input)['logits']
                                # B L 1 26
                                ...
                                # for cnt in range(len(readed_txts)):
                                #     print(readed_txts[cnt], ';',lm_txts[cnt], ';',label_strs[cnt])
                                visual_clues = None
                                # visual_clues = []
                                ...
                                if True:
                                    ...
                                    loss_recog_distill_each = loss_recog_distill_each.mean()
                                    loss_recog_distill += loss_recog_distill_each
                                # loss_recog_distill += loss_lm_distill  # * (1 + 0.5 * i)
                                # add the loss
                                # loss_img += loss_ctc_lm
                                # add noise
                            ...
                            loss_img_each = image_crit(cascade_images, images_hr, label_strs = label_strs).mean() * 100
                        ...
                        if self.args.arch in ["srcnn", "rdn", "vdsr"] and not self.args.mask:
                            channel_num = 3
                        else:
                            channel_num = 4
        ...
        for i, data in (enumerate(val_loader)):
            ...
            for batch_i in range(len(images_lr)):
                ...
                if vis:
                    ...
                    # cv2.imwrite(os.path.join(vis_dir, file_name), cv2.cvtColor(sr.astype(np.uint8), cv2.COLOR_RGB2BGR))
                    # file_name2 = str(self.global_img_val_cnt)+"_"+ label.lower() + '_lr.jpg'
                    # cv2.imwrite(os.path.join(vis_dir, file_name2), cv2.cvtColor(lr.astype(np.uint8), cv2.COLOR_RGB2BGR))
                    #
                    # if lr_wrong and not sr_wrong:
